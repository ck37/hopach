...
% \VignetteDepends{tools, cluster, Biobase}
...
<<geneSelection,eval=FALSE,echo=TRUE>>=
...
<<hopachGene,eval=FALSE,echo=TRUE>>=
...
dplot(gene.dist,gene.hobj,ord="final",main="Golub AML/ALL Data (1999): Gene Distance Matrix",showclusters=FALSE)  #ordered distance matrix
@
...
<<bootstrap,eval=FALSE,echo=TRUE>>=
bobj<-boothopach(golub.subset,gene.hobj,I=100)
@

The argument \Rfunarg{I} controls the number of bootstrap resmapled data sets used. The default value is \texttt{I=1000}, which represents a balance between precision and speed. For this example, we use only \texttt{I=100} so that it will not run too long. The \Rfunction{boothopach} function still takes some time to run. The bootstrap is a powerful, but computationally intensive, method. The \Rfunction{bootplot} function makes a barplot of the bootstrap reappearance proportions (See Figure \ref{barplot}). 
...
<<hopachArray,eval=FALSE,echo=TRUE>>=
